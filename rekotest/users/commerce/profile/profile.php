...
<div class="innerContainerPrdOverview">
  ...
  <h2> Din profil</h2>
    <form method="post" enctype="multipart/form-data" action="">
      ...
      <?php
      if(!$image){
        print("<img src='/img/default-profile.jpg' height='100px'/></a>");
      }
      else{
        print("<img src='/img/users/".$userID."/".$image."' height='100px'/>");
      }
    ...
    <button class="Passordchange" onclick="window.location.href = 'http://reko.opheim.as/users/commerce/profile/editPassword.php';">Endre passord</button>
    <?php
        ...
        if($fileName){
                ...
                $newPath="/img/users/".$userID.'/'.$fileName;
        

                if($fileType != "image/gif" && $fileType != "image/jpeg" && $fileType != "image/jpg" && $fileType != "image/png" )
                {
                  print("<meta http-equiv='refresh' content='0;URL=http://reko.opheim.as/users/commerce/profile/profile.php?error=picture'/>") and die;
                }
                ...
                if($profileIMG != $fileName)
                {
                    ...
                    @unlink($path.$image);
                    move_uploaded_file($tmpName, $newPath) or ("<meta http-equiv='refresh' content='0;url=http://reko.opheim.as/users/commerce/products/editProduct.php?error=server'>") and die;
                    ...
                    if(mysqli_query($db,$sql))
                        {
                          print("<meta http-equiv='refresh' content='0;URL=http://reko.opheim.as/users/commerce/profile/profile.php?success=updateOK'/>");
                        }
                    else
                        {
                          print("<meta http-equiv='refresh' content='0;URL=http://reko.opheim.as/users/commerce/profile/profile.php?error=sql'/>");
                          unlink($newPath) or ("<meta http-equiv='refresh' content='0;URL=http://reko.opheim.as/users/commerce/profile/profile.php?error=server'/>'>") and die;
                        }
                }
                if($profileIMG == $fileName)
                  {
                      $sql= "UPDATE users SET firstName ='$firstName1',lastName ='$lastName',email='$email1',phoneNumber='$phone1' WHERE userID='$userID';";
                      mysqli_query($db,$sql) or ("<meta http-equiv='refresh' content='0;URL=http://reko.opheim.as/users/commerce/profile/profile.php?error=sql'/>") and die;
                      print("<meta http-equiv='refresh' content='0;URL=http://reko.opheim.as/users/commerce/profile/profile.php?success=updateOK'/>");
                  }
        ...
        if(isset($_POST['delete'])){
          ...
          if($result){
            print("<meta http-equiv='refresh' content='0;URL=http://reko.opheim.as/access/logout.php'/>");
          }
          else{
            print("<meta http-equiv='refresh' content='0;URL=http://reko.opheim.as/users/customer/profile/profile.php?error=delete'/>");
          }
