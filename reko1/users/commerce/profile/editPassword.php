...
?>
            ...
            <?php
                if(isset($_POST['change'])){
                    ...
                    if(!$result){
                        print("<meta http-equiv='refresh' content='0;url=http://opheimpi.zapto.org/www/sda/reko/users/commerce/profile/editPassword.php?error=loginerror'>");
                    }
                    if($newPassword != $renewPassword){
                        print("<meta http-equiv='refresh' content='0;url=http://opheimpi.zapto.org/www/sda/reko/users/commerce/profile/editPassword.php?error=wrong'>");
                    }
                    if($newPassword == $password){
                        print("<meta http-equiv='refresh' content='0;url=http://opheimpi.zapto.org/www/sda/reko/users/commerce/profile/editPassword.php?error=equals'>");
                    }
                    else{
                        $cryptPassword = password_hash($newPassword,PASSWORD_DEFAULT);
                        $sql = "UPDATE users SET password='$cryptPassword' WHERE userID=$userID;";
                        mysqli_query($db,$sql) or ("<meta http-equiv='refresh' content='0;url=http://opheimpi.zapto.org/www/sda/reko/users/commerce/profile/editPassword.php?error=sql'>") and die;
                        print("<meta http-equiv='refresh' content='0;url=http://opheimpi.zapto.org/www/sda/reko/users/commerce/profile/editPassword.php?success=updateOK'>");
                    }
